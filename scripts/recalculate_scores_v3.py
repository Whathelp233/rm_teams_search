...
def main():
    ...
    index["schema_version"] = "3.5.0"
    index["data_version"] = "score-3.5.0"
    ...
    index["scoring_notice"] = "六维3.5：按逐赛区滚动时间外表现重校综合权重；适应保留独立展示但降低对胜率的干扰"
    for team, (path, payload) in payloads.items():
        ...
        payload["schema_version"] = "3.5.0"; payload["data_version"] = "score-3.5.0"
        ...
        for name, score in scores.items():
            ...
            detail.update({"version": "3.5.0", "method": method})
        ...
        payload["strength_analysis"] = {
            "version": "3.5.0", "score": rounded(strength), "tactical_score": rounded(tactical),
    ...
    print(f"recalculated {len(teams)} teams with score schema 3.5.0")
