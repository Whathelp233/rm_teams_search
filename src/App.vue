...
const dataRevision = 'score-3.3.0'
...
<div class="shell">
  ...
  <main v-if="selected">
    ...
    <section class="panel">
      <div class="section-head"><h2>高维战术解析 3.3</h2><span>逐局净事实 · 条件校正 · 时间回测</span></div>
      ...
      <div class="insight-grid">
        ...
        <article class="insight-card strength-card"><h3>综合强度 3.3 · {{show(dimensions.strength?.score)}}</h3><div class="metric-pairs"><span>六维战术 · 90%<strong>{{show(dimensions.strength?.tactical_score)}}</strong></span><span>赛程校正赛果 · 10%<strong>{{show(dimensions.strength?.result_score)}}</strong></span><span>赛程强度参数<strong>{{show(dimensions.strength?.schedule_rating)}}</strong></span><span>数据置信度（不计分）<strong>{{show(dimensions.confidence?.overall,'%')}}</strong></span><span>有效对局<strong>{{dimensions.confidence?.games||0}} 局</strong></span><span>验证方式<strong>逐赛区滚动时间回测</strong></span></div></article>
      </div>
