...
const tacticCards = computed(() => {
  ...
  return [
    ...
    card('adaptability', '适应能力', d.analysis.adaptability,
      [{ label:'红蓝迁移差', value:signed(d.analysis.adaptability?.raw?.side_transfer,' 分',2), meta:`${sampleCount('adaptability','side_transfer')} 个成对样本` }, { label:'跨对手下限残差', value:signed(d.analysis.adaptability?.raw?.opponent_robustness,' 分',2), meta:`${sampleCount('adaptability','opponent_robustness')} 个对手` }, { label:'强敌超预期残差', value:signed(d.analysis.adaptability?.raw?.strong_opponent_residual,' 分',2), meta:`${sampleCount('adaptability','strong_opponent_residual')} 局` }, { label:'前哨失守后调整', value:signed(d.analysis.adaptability?.raw?.setback_adjustment,'×',3), meta:`${sampleCount('adaptability','setback_adjustment')} 局` }],
      [{ label:'败局后再战变化', value:signed(d.analysis.adaptability?.raw?.rematch_adjustment,' 分',2), meta:`${sampleCount('adaptability','rematch_adjustment')} 组复战` }],
      [component(d.analysis.adaptability,'红蓝方迁移','side_transfer','10%'), component(d.analysis.adaptability,'跨对手下限','opponent_robustness','10%'), component(d.analysis.adaptability,'强敌超预期','strong_opponent_residual','20%'), component(d.analysis.adaptability,'局内失守后调整','setback_adjustment','30%'), component(d.analysis.adaptability,'败局后再战调整','rematch_adjustment','30%')], '正值代表相对基线改善；缺样本时回归中性'),
  ]
})
...
const methodologySections = computed(() => {
  ...
  const common = { title:'共同口径', items:['数据基于规则手册实场图与通信协议官方坐标。判罚、撞击和定位覆盖不进入战术得分。','综合强度由六维战术事实 90% 与赛程校正赛果 10% 构成；证据覆盖影响置信度，不直接参与得分。'] }
  const byTab = {
    overview:[{ title:'评分阅读', items:['六维图展示全局排名与证据强度；分数表示同批队伍中的相对表现，不等于确定赛果。'] }],
    ...
    compare:[{ title:'胜率模型', items:['胜率来自区域赛样本模型并展示估计区间。透明对手分只解释赛程，不重复叠加；直接交手仅作背景。'] }],
  }
...
<div class="app" :data-density="density">
  ...
  <main v-else-if="selected" :aria-busy="loadingTeam">
    ...
    <section v-if="teamTab==='tactics'" class="panel">
      <div class="section-head tactic-section-head"><h2>高维战术解析 3.9</h2><button @click="toggleAllTactics">{{allTacticsExpanded?'收起全部':'展开全部'}}</button></div>
      <div class="insight-grid">
        ...
        <article class="insight-card strength-card"><h3>综合强度 3.9 · {{show(dimensions.strength?.score)}}</h3><div class="metric-pairs"><span>六维战术<strong>{{show(dimensions.strength?.tactical_score)}}</strong></span><span>赛程校正赛果<strong>{{show(dimensions.strength?.result_score)}}</strong></span><span>赛程强度参数<strong>{{show(dimensions.strength?.schedule_rating)}}</strong></span><span>透明对手分<strong>{{show(dimensions.opponentScore?.score)}} · 赛区第 {{show(dimensions.opponentScore?.region_rank)}}/{{show(dimensions.opponentScore?.region_size)}}</strong></span><span>对手去直接交手胜率<strong>{{show(dimensions.opponentScore?.opponent_win_rate_pct,'%')}}</strong></span><span>二阶对手胜率<strong>{{show(dimensions.opponentScore?.opponent_opponent_win_rate_pct,'%')}}</strong></span><span>不同对手数<strong>{{show(dimensions.opponentScore?.unique_opponents)}} 个</strong></span><span>数据置信度<strong>{{show(dimensions.confidence?.overall,'%')}}</strong></span></div></article>
      </div>
    ...
    <section v-if="teamTab==='tactics'" class="panel tactical-mode-panel">
      <div class="section-head"><div><h2>阶段动作链</h2><span v-if="tacticalProfile">{{tacticalProfile.sample.games}} 局 · {{tacticalProfile.sample.opponents}} 个对手 · 17 类模式</span></div><label class="insufficient-toggle"><input type="checkbox" v-model="includeInsufficientPatterns">显示样本不足</label></div>
      ...
      <div v-if="loadingTactics" class="empty-state">正在生成阶段画像…</div>
      ...
      <div v-else class="tactical-pattern-list"><TacticalPatternCard v-for="pattern in tacticalPatterns" :key="pattern.pattern_id" :pattern="pattern" :expanded="expandedPatterns.has(pattern.pattern_id)" @toggle="togglePattern(pattern.pattern_id)" @evidence="openTacticalEvidence"/><div v-if="!tacticalPatterns.length" class="empty-state">当前阶段没有达到展示门槛的动作链。</div></div>
    </section>
    <section v-if="teamTab==='tactics'" class="panel counter-lab">
      <div class="section-head"><div><h2>双队定制克制</h2><span>{{selected.team}}作为目标队伍</span></div><label>我方执行队伍<select v-model="counterExecutorSlug"><option value="">选择队伍</option><option v-for="team in index.teams.filter(team=>team.team!==selected.team)" :key="team.slug" :value="team.slug">{{team.team}}</option></select></label></div>
      ...
      <template v-else><div class="counter-matchup-head"><b>{{counterExecutor?.team}}</b><span>执行</span><b>{{selected.team}}</b><small>优先级按规则胜负影响、复现度、触发时间与证据强度计算</small></div><div class="counter-plan-list"><CounterPlanCard v-for="(plan,indexAt) in counterPlans" :key="plan.id" :plan="plan" :index="indexAt" @evidence="openTacticalEvidence"/></div><div v-if="!counterPlans.length" class="empty-state">当前样本不足以形成双队方案。</div></template>
    </section>
