...
const dataRevision = 'score-3.1.0'
...
<div class="shell">
  ...
  <main v-if="selected">
    ...
    <section class="panel">
      <div class="section-head"><h2>高维战术解析 3.1</h2><span>逐局净事实 · 条件校正 · 小样本收缩</span></div>
      ...
      <div class="insight-grid">
        ...
        <article class="insight-card defense-card"><h3>防守韧性 · {{show(dimensions.analysis.defense?.score)}}</h3><div class="metric-pairs"><span>校正伤害交换 · 25%<strong>{{show(dimensions.analysis.defense?.components?.trade_resilience)}}</strong></span><span>机器人韧性 · 20%<strong>{{show(dimensions.analysis.defense?.components?.mobile_resilience)}}</strong></span><span>前哨拒止 · 20%<strong>{{show(dimensions.analysis.defense?.components?.outpost_denial)}}</strong></span><span>基地拒止 · 25%<strong>{{show(dimensions.analysis.defense?.components?.base_denial)}}</strong></span><span>崩盘抵抗 · 10%<strong>{{show(dimensions.analysis.defense?.components?.collapse_resistance)}}</strong></span><span>聚合口径<strong>典型表现 70% · 下四分位 30%</strong></span><span>已排除噪声<strong>判罚 {{show(dimensions.analysis.defense?.excluded?.penalty_damage)}} · 撞击 {{show(dimensions.analysis.defense?.excluded?.collision_damage)}}</strong></span></div></article>
        ...
        <article class="insight-card"><h3>适应能力 · {{show(dimensions.analysis.adaptability?.score)}}</h3><div class="metric-pairs"><span>红蓝方迁移 · 25%<strong>{{show(dimensions.analysis.adaptability?.components?.side_transfer)}}</strong></span><span>跨对手下限 · 20%<strong>{{show(dimensions.analysis.adaptability?.components?.opponent_robustness)}}</strong></span><span>强敌超预期 · 25%<strong>{{show(dimensions.analysis.adaptability?.components?.strong_opponent_residual)}}</strong></span><span>前哨失守后调整 · 20%<strong>{{show(dimensions.analysis.adaptability?.components?.setback_adjustment)}}</strong></span><span>有效策略切换 · 10%<strong>{{show(dimensions.analysis.adaptability?.components?.viable_variants)}}</strong></span><span>缺失条件样本<strong>只回归中性，不加减分</strong></span></div></article>
