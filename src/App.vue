...
const dataRevision = 'score-3.9.1-role-data-1.1.0-replay-3.1.0-tournament-3.0.0-tactics-1.0.0'
  ...
  <main v-else-if="selected" :aria-busy="loadingTeam">
    ...
    <section v-if="teamTab==='tactics'" class="panel">
      ...
      <div class="insight-grid">
        ...
        <article class="insight-card strength-card"><h3>综合强度 3.9.1 · {{show(dimensions.strength?.score)}}</h3><div class="metric-pairs"><span>六维战术<strong>{{show(dimensions.strength?.tactical_score)}}</strong></span><span>赛程校正赛果<strong>{{show(dimensions.strength?.result_score)}}</strong></span><span>原始胜率<strong>{{show(dimensions.strength?.raw_win_rate_pct,'%')}}</strong></span><span>对手校正参数<strong>{{show(dimensions.strength?.schedule_rating)}}</strong></span><span>赛程难度证据<strong>{{show(dimensions.opponentScore?.score)}} · 赛区第 {{show(dimensions.opponentScore?.region_rank)}}/{{show(dimensions.opponentScore?.region_size)}}</strong></span><span>对手去直接交手胜率<strong>{{show(dimensions.opponentScore?.opponent_win_rate_pct,'%')}}</strong></span><span>二阶对手胜率<strong>{{show(dimensions.opponentScore?.opponent_opponent_win_rate_pct,'%')}}</strong></span><span>数据置信度<strong>{{show(dimensions.confidence?.overall,'%')}}</strong></span></div><details v-if="dimensions.opponentScore?.breakdown?.length" class="opponent-breakdown"><summary>逐个对手校正证据（{{dimensions.opponentScore.breakdown.length}}队）</summary><div><span v-for="opponent in dimensions.opponentScore.breakdown" :key="opponent.opponent"><b>{{opponent.opponent}}</b><small>交手 {{opponent.meetings}} 局 · 排除直接交手后 {{opponent.opponent_other_wins}}/{{opponent.opponent_other_games}} · 平滑胜率 {{opponent.adjusted_win_rate_pct}}% · 二阶 {{opponent.second_order_pct}}%</small></span></div><p>赛果分已通过 Bradley–Terry 联立校正对手；赛程难度不再重复加分。</p></details></article>
      </div>
