...
const dataRevision = 'score-3.2.0'
...
<div class="shell">
  ...
  <main v-if="selected">
    ...
    <section class="panel">
      <div class="section-head"><h2>高维战术解析 3.2</h2><span>逐局净事实 · 条件校正 · 小样本收缩</span></div>
      ...
      <div class="insight-grid">
        ...
        <article class="insight-card"><h3>适应能力 · {{show(dimensions.analysis.adaptability?.score)}}</h3><div class="metric-pairs"><span>红蓝方迁移 · 25%<strong>{{show(dimensions.analysis.adaptability?.components?.side_transfer)}}</strong></span><span>跨对手下限 · 20%<strong>{{show(dimensions.analysis.adaptability?.components?.opponent_robustness)}}</strong></span><span>强敌超预期 · 25%<strong>{{show(dimensions.analysis.adaptability?.components?.strong_opponent_residual)}}</strong></span><span>前哨失守后调整 · 20%<strong>{{show(dimensions.analysis.adaptability?.components?.setback_adjustment)}}</strong></span><span>策略切换效果 · 10%<strong>{{show(dimensions.analysis.adaptability?.components?.strategy_switch_effect)}}</strong></span><span>缺失条件样本<strong>只回归中性，不加减分</strong></span></div></article>
