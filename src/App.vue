...
const dataRevision = 'score-3.0.0'
...
function renderCharts() {
  ...
  scoreChart.setOption({radar:{center:['50%','52%'],radius:'57%',indicator:['净战斗火力','目标转化','空间控制','防守韧性','资源转化','适应能力'].map(name=>({name,max:100})),axisName:{color:'#cbd5e1',fontSize:13},splitArea:{areaStyle:{color:['#13233a','#0e1a2d']}}},series:[{type:'radar',data:[{value:keys.map(k=>selected.value.scores[k])}],areaStyle:{color:'#22d3ee66'},lineStyle:{color:'#22d3ee'}}],textStyle:{color:'#cbd5e1'}})
...
<div class="shell">
  ...
  <main v-if="selected">
    ...
    <section class="panel">
      <div class="section-head"><h2>高维战术解析 3.0</h2><span>逐局净事实 · 统一尺度 · 小样本收缩</span></div>
      <p class="method coverage-note"><b>评分边界：</b>判罚、撞击和定位覆盖不进入战术得分；雷达仅使用原始库可验证的反制事件。每项同时展示分项得分、原始事实和有效样本。</p>
      <div class="insight-grid">
        <article class="insight-card"><h3>净战斗火力 · {{show(dimensions.analysis.firepower?.score)}}</h3><div class="metric-pairs"><span>对手校正净输出 · 35%<strong>{{show(dimensions.analysis.firepower?.components?.clean_output)}}</strong></span><span>机器人命中效率 · 20%<strong>{{show(dimensions.analysis.firepower?.components?.accuracy)}}</strong></span><span>阵亡转化 · 25%<strong>{{show(dimensions.analysis.firepower?.components?.kill_conversion)}}</strong></span><span>持续施压 · 20%<strong>{{show(dimensions.analysis.firepower?.components?.pressure_uptime)}}</strong></span><span>净输出相对预期<strong>{{show(dimensions.analysis.firepower?.raw?.clean_output,'×')}}</strong></span><span>机器人命中率<strong>{{show(Math.round((dimensions.analysis.firepower?.raw?.accuracy||0)*1000)/10,'%')}}</strong></span></div></article>
        <article class="insight-card"><h3>目标转化 · {{show(dimensions.analysis.objective?.score)}}</h3><div class="metric-pairs"><span>前哨施压 · 20%<strong>{{show(dimensions.analysis.objective?.components?.outpost_pressure)}}</strong></span><span>前哨转化 · 25%<strong>{{show(dimensions.analysis.objective?.components?.outpost_conversion)}}</strong></span><span>基地施压 · 20%<strong>{{show(dimensions.analysis.objective?.components?.base_pressure)}}</strong></span><span>基地转化 · 25%<strong>{{show(dimensions.analysis.objective?.components?.base_conversion)}}</strong></span><span>战略工具 · 10%<strong>{{show(dimensions.analysis.objective?.components?.strategic_tools)}}</strong></span><span>首伤前哨中位<strong>{{dimensions.objective.firstOutpostSec==null?'—':fmtSecond(dimensions.objective.firstOutpostSec)}}</strong></span><span>中位击毁耗时<strong>{{dimensions.objective.medianOutpostKillSec==null?'—':fmtSecond(dimensions.objective.medianOutpostKillSec)}}</strong></span><span>有效击毁样本<strong>{{dimensions.objective.outpostKillGames}} 局</strong></span></div></article>
        <article class="insight-card"><h3>空间控制 · {{show(dimensions.analysis.spatial?.score)}}</h3><div class="metric-pairs"><span>相对领土纵深 · 30%<strong>{{show(dimensions.analysis.spatial?.components?.relative_territory)}}</strong></span><span>前压在场 · 25%<strong>{{show(dimensions.analysis.spatial?.components?.forward_presence)}}</strong></span><span>中区相对控制 · 20%<strong>{{show(dimensions.analysis.spatial?.components?.neutral_control)}}</strong></span><span>场地覆盖 · 15%<strong>{{show(dimensions.analysis.spatial?.components?.field_coverage)}}</strong></span><span>时长归一化机动 · 10%<strong>{{show(dimensions.analysis.spatial?.components?.mobility)}}</strong></span><span>相对推进纵深<strong>{{show(dimensions.analysis.spatial?.raw?.relative_territory,' m')}}</strong></span><span>前压车秒占比<strong>{{show(Math.round((dimensions.analysis.spatial?.raw?.forward_presence||0)*1000)/10,'%')}}</strong></span><span>定位覆盖（仅置信度）<strong>{{show(dimensions.confidence?.position_coverage_pct,'%')}}</strong></span></div></article>
        <article class="insight-card defense-card"><h3>防守韧性 · {{show(dimensions.analysis.defense?.score)}}</h3><div class="metric-pairs"><span>对手校正承伤 · 25%<strong>{{show(dimensions.analysis.defense?.components?.combat_containment)}}</strong></span><span>机器人血量面积 · 20%<strong>{{show(dimensions.analysis.defense?.components?.mobile_integrity)}}</strong></span><span>机器人在线率 · 15%<strong>{{show(dimensions.analysis.defense?.components?.unit_availability)}}</strong></span><span>前哨血量面积 · 15%<strong>{{show(dimensions.analysis.defense?.components?.outpost_integrity)}}</strong></span><span>基地血量与首伤 · 25%<strong>{{show(dimensions.analysis.defense?.components?.base_protection)}}</strong></span><span>已排除噪声<strong>判罚 {{show(dimensions.analysis.defense?.excluded?.penalty_damage)}} · 撞击 {{show(dimensions.analysis.defense?.excluded?.collision_damage)}}</strong></span></div></article>
        <article class="insight-card"><h3>资源转化 · {{show(dimensions.analysis.resource?.score)}}</h3><div class="metric-pairs"><span>经济获取 · 20%<strong>{{show(dimensions.analysis.resource?.components?.acquisition)}}</strong></span><span>经济使用 · 15%<strong>{{show(dimensions.analysis.resource?.components?.utilization)}}</strong></span><span>战斗转化 · 25%<strong>{{show(dimensions.analysis.resource?.components?.combat_conversion)}}</strong></span><span>目标转化 · 25%<strong>{{show(dimensions.analysis.resource?.components?.objective_conversion)}}</strong></span><span>热控效率 · 15%<strong>{{show(dimensions.analysis.resource?.components?.thermal_efficiency)}}</strong></span><span>经济使用率<strong>{{show(Math.round((dimensions.analysis.resource?.raw?.utilization||0)*1000)/10,'%')}}</strong></span></div></article>
        <article class="insight-card"><h3>适应能力 · {{show(dimensions.analysis.adaptability?.score)}}</h3><div class="metric-pairs"><span>红蓝方平衡 · 20%<strong>{{show(dimensions.analysis.adaptability?.components?.side_balance)}}</strong></span><span>多对手稳健 · 15%<strong>{{show(dimensions.analysis.adaptability?.components?.opponent_robustness)}}</strong></span><span>强敌表现 · 30%<strong>{{show(dimensions.analysis.adaptability?.components?.strong_opponent)}}</strong></span><span>前哨失守后响应 · 25%<strong>{{show(dimensions.analysis.adaptability?.components?.setback_response)}}</strong></span><span>有效战术选项 · 10%<strong>{{show(dimensions.analysis.adaptability?.components?.option_coverage)}}</strong></span></div></article>
        ...
        <article class="insight-card strength-card"><h3>综合强度 3.0 · {{show(dimensions.strength?.score)}}</h3><div class="metric-pairs"><span>六维战术 · 75%<strong>{{show(dimensions.strength?.tactical_score)}}</strong></span><span>赛程校正赛果 · 25%<strong>{{show(dimensions.strength?.result_score)}}</strong></span><span>赛程强度参数<strong>{{show(dimensions.strength?.schedule_rating)}}</strong></span><span>数据置信度（不计分）<strong>{{show(dimensions.confidence?.overall,'%')}}</strong></span><span>有效对局<strong>{{dimensions.confidence?.games||0}} 局</strong></span><span>赛果模型<strong>正则化 Bradley–Terry</strong></span></div></article>
      </div>
